...
TEST_CASE("basic heating computation", "[calc]")
{
    ...
    TEST_ASSERT_GREATER_THAN(0.0f, result.heating.heater_power_catalog_w);
    TEST_ASSERT_GREATER_THAN(0.0f, result.heating.enclosure_volume_l);
    TEST_ASSERT_TRUE(result.lighting.valid);
    TEST_ASSERT_GREATER_THAN(0u, result.lighting.led_count);
}
